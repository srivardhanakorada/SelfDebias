...
# ORIGINAL (Vanilla)
all_original = []
# for i in tqdm(range(0, NUM_IMAGES, BATCH_SIZE),desc="Vanilla"):
#     result,_ = pipeline(
#         prompt=PROMPT,
...
for i in range(0, NUM_IMAGES, BATCH_SIZE):
    ...
    all_original.extend(result.images)
save_images(all_original, "our_outputs/original", "original")
...
#     torch.save(grad_list,f"gradients/theirs/grad_{i}.pt")
for i in range(0, NUM_IMAGES, BATCH_SIZE):
    result,grad_list = pipeline(
        prompt=PROMPT,
        generator=torch.Generator(device).manual_seed(seed + i),
    ...
    all_debiased.extend(result.images)
    torch.save(grad_list,f"gradients/ours/grad_{i}.pt")
save_images(all_debiased, "our_outputs/debiased", "debiased")