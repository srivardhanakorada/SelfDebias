...
os.makedirs(FOLDER, exist_ok=True)
os.makedirs(FOLDER+"/cluster_consistency_plot",exist_ok=True)

pipeline = DDIMPipeline.from_pretrained("google/ddpm-celebahq-256", torch_dtype=torch.float16,folder = FOLDER).to(device)
...
for i in tqdm(range(0, NUM_IMAGES, BATCH_SIZE),desc="Debiased"):    
    ...
    save_batch(result.images, FOLDER+"/debiased_ddim/",start_idx = i,prefix = "debiased")    
...
for i in tqdm(range(0, NUM_IMAGES, BATCH_SIZE),desc="Original"):
    ...
    save_batch(result.images, FOLDER+"/original_ddim/", start_idx = i, prefix="original")
