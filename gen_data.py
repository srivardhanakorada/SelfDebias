...
BATCH_SIZE = 100
PROMPT = "A ultra realistic photo of a natural environment, single kind, no humans or animals, only one"
...
os.makedirs("train_data/h",exist_ok=True)
os.makedirs("train_data/clip",exist_ok=True)
...
for img_id in tqdm(range(args.start, args.end),desc="Vanilla"):
    ...
    img = result.images[0]
    img.save(f"train_data/images/{img_id:05d}.png")
    h_list = [h_vecs[t][0].half().cpu() for t in range(len(h_vecs))]
    temp = [h_vecs[t][1].half().cpu() for t in range(len(h_vecs))]
    h_list.extend(temp)
    torch.save(h_list, f"train_data/h/{img_id:05d}.pt")
    image_input = clip_preprocess(Image.open(f"train_data/images/{img_id:05d}.png")).unsqueeze(0).to(device)
    with torch.no_grad(): clip_vec = clip_model.encode_image(image_input).squeeze().half().cpu()
    torch.save(clip_vec, f"train_data/clip/{img_id:05d}.pt")